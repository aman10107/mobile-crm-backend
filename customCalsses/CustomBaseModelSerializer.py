...
# customClasses/ShopSerializerMixin.py
from rest_framework import serializers

class ShopSerializerMixin:
    """
    Mixin to make shop field read-only with default value
    """
    def get_fields(self):
        fields = super().get_fields()
        if 'shop' in fields:
            fields['shop'].read_only = True
            fields['shop'].required = False
        return fields
...
class CustomBaseModelSerializer(ShopSerializerMixin, DynamicExpandMixin, serializers.ModelSerializer):
    pass
